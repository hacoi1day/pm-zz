  ...
  >
            ...
            <b-table-simple id="table-data" hover>
              ...
              <b-tbody>
                <b-tr v-for="(item, index) in items" :key="index" @click="openModalRequest(item.id)">
                  <b-td>{{ item.user ? item.user.name : '' }}</b-td>
                  <b-td>{{ item.user ? item.user.email : '' }}</b-td>
    ...
    <b-modal ref="modalRequest" hide-footer title="Thông tin yêu cầu">
      <div class="d-block" v-if="requestSelected">
        <p><strong>Họ tên:</strong> {{ requestSelected.user ? requestSelected.user.name : '' }}</p>
        <p><strong>Email:</strong> {{ requestSelected.user ? requestSelected.user.email : '' }}</p>
...
import { getRequest, listRequest, approvalRequest, refuseRequest } from '../../apis/request';
...
export default {
  ...
  methods: {
    async fetchRequestList () {
      const {data, last_page, total} = await listRequest(this.currentPage, this.currentStatusId);
      this.items = data;
